...
def success(request):
    try:
        ...
        if request.method == 'POST':
            if request.POST.get('submit') == str(lst[-1]):
                ...
                if (getuser.condition_for_getassured):
                    ...
                    if(getuser.getassured_attemptno==1):
                        ...
                        if (userinput1 == pre_question.correct_answer): #attempt 1: correct
                            score = 10
                            bool = True
                            respo.selected_answer=userinput1
                            respo.score=score
                            getuser.getassured_attemptno = 0
                            respo.save()
                            ...
                            getuser.save()
                        else:                                           #attempt 1: wrong
                            score = -8
                            bool = True
                            respo.selected_answer = userinput1
                            respo.score = score
                            getuser.getassured_attemptno = 2
                            respo.save()
                            getuser.total_score += respo.score
                            #lst.pop()
                            getuser.bool = bool
                            getuser.save()
                            return render(request, 'task2part2temp/question.html', {'user': getuser, 'question': pre_question, 'time': [msg2]})

                    elif(getuser.getassured_attemptno==2):
                        userinput2 = request.POST['user_ans']
                        if (userinput2 == pre_question.correct_answer): #attempt 2: correct
                            score = 8
                            bool = True

                        else:                                           #attempt 2: wrong
                            score = -10
                            bool = False
                            getuser.getassured_attemptno = 1 # refill for next time #to be done later
                        respo.selected_answer = userinput2
                        respo.score = score
                        getuser.getassured_attemptno = 0 #refill
                        respo.save()
                        getuser.total_score += respo.score
                        lst.pop()
                        getuser.bool = bool
                ...
                #normal code
                    ...
                    respo.save()
                    getuser.total_score += respo.score
                    getuser.total_percentage += respo.percentage
                    lst.pop()
                    getuser.bool = bool
                    getuser.save()
        ...
        getuser.save()
        return render(request, 'task2part2temp/question.html', {'user': getuser, 'question': question, 'time': [msg2],'increment':increment})
    except:
